...
/**
 * 对android.support.v4.widget.SwipeRefreshLayout扩展了上拉加载更多功能
 * Created by Lynn on 2016-7-21.
 */

public class SwipeRefreshPlusLayout extends ViewGroup implements NestedScrollingParent,
        NestedScrollingChild
{
    ...
    private static final String TAG = SwipeRefreshPlusLayout.class.getSimpleName();
    ...
    private static final int LOAD_MORE_ANIMATION_DURATION = 150;
    ...
    private int mLoadMoreViewIndex = -1;
     ...
     * @param context
     */
    public SwipeRefreshPlusLayout(Context context)
    {
        this(context, null);
    }
    ...
    {
        if (mLoadMoreViewIndex < 0)
        {
            return i;
        } else if (i == childCount - 1)
        {
            // Draw the selected child last
            return mLoadMoreViewIndex;
        } else if (i >= mLoadMoreViewIndex)
        {
     ...
     * @param colorResIds
     */
    public void setLoadMoreColorSchemeResources(@ColorRes int... colorResIds)
    {
        ...
        setLoadMoreColorSchemeColors(colorRes);
    }
     ...
     * @param colors
     */
    public void setLoadMoreColorSchemeColors(@ColorInt int... colors)
    {
        ensureTarget();
        mLoadMoreProgress.setColorSchemeColors(colors);
    }
    ...
    private void ensureTarget()
    {
        ...
        if (mTarget == null)
        {
            for (int i = 0; i < getChildCount(); i++)
            {
                ...
                if (!child.equals(mCircleView) && !child.equals(mLoadMoreView))
                {
                    mTarget = child;
                    break;
                }
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom)
    {
        ...
        if (mEnableLoadMore)
        {
            int loadMoreHeight = mLoadMoreView.getMeasuredHeight();
            mLoadMoreView.layout(childLeft, childTop + childHeight, childLeft + childWidth, loadMoreHeight + childTop + childHeight);
        }
    }
    ...
    public void onMeasure(int widthMeasureSpec, int heightMeasureSpec)
    {
        ...
        mTarget.measure(MeasureSpec.makeMeasureSpec(
                ...
                MeasureSpec.EXACTLY), MeasureSpec.makeMeasureSpec(targetHeight
                , MeasureSpec.EXACTLY));
        mCircleView.measure(MeasureSpec.makeMeasureSpec(mCircleWidth, MeasureSpec.EXACTLY),
                MeasureSpec.makeMeasureSpec(mCircleHeight, MeasureSpec.EXACTLY));

        LayoutParams lp = mLoadMoreView.getLayoutParams();
        mLoadMoreView.measure(MeasureSpec.makeMeasureSpec(
                getMeasuredWidth() - getPaddingLeft() - getPaddingRight(),
                MeasureSpec.EXACTLY), MeasureSpec.makeMeasureSpec(lp.height, MeasureSpec.EXACTLY));
        ...
        mLoadMoreViewIndex = -1;
        for (int index = 0; index < getChildCount(); index++)
        {
            if (getChildAt(index) == mLoadMoreView)
            {
                mLoadMoreViewIndex = index;
                break;
            }
        }
    }
    ...
    public boolean canChildScrollUp()
    {
        ...
        if (android.os.Build.VERSION.SDK_INT < 14)
        {
            if (mTarget instanceof AbsListView)
            {
                ...
                can = absListView.getChildCount() > 0
                        && (absListView.getFirstVisiblePosition() > 0 || absListView.getChildAt(0)
                        .getTop() < absListView.getPaddingTop());
            } else
            {
                can = ViewCompat.canScrollVertically(mTarget, -1) || mTarget.getScrollY() > 0;
            }
        } else
        {
            can = ViewCompat.canScrollVertically(mTarget, -1);
        }
        return can;
    }
        ...
        if (android.os.Build.VERSION.SDK_INT < 14)
        {
            if (mTarget instanceof AbsListView)
            {
                ...
                can = absListView.getChildCount() > 0
                        ...
                        || mTarget.getMeasuredHeight() - absListView.getChildAt(absListView.getChildCount() - 1)
     ...
     * @return
     */
    private boolean canItemsFillParent()
    {
        boolean can = true;
        if (mTarget instanceof ViewGroup)
        {
            ...
            can = child.getBottom() >= mTarget.getMeasuredHeight() ? true : false;
        }
        return can;
    }
    ...
    {
        if (mEnableLoadMore && canItemsFillParent())
        {
            if (canChildScrollDown() && mLoadMoreViewMoveDistance >= 0 || isRefreshing())
            {
                mPullDistance = 0;
                mLoadMoreView.setTranslationY(0);
                mTarget.setTranslationY(0);
            ...
            {
                mLoadMoreViewMoveDistance += mPullDistance;
                if (mLoadMoreViewMoveDistance > 0)
                {
                    mLoadMoreViewMoveDistance = 0;
                }
                if (Math.abs(mLoadMoreViewMoveDistance) > mLoadMoreView.getMeasuredHeight())
                {
                    mLoadMoreViewMoveDistance = -mLoadMoreView.getMeasuredHeight();
                } else
                {
                    mLoadMoreView.setTranslationY((int) mLoadMoreViewMoveDistance);
                    mTarget.setTranslationY((int) mLoadMoreViewMoveDistance);
                }
            }
        }
